...
struct rtc_sw_request {
        int status;                   /* status of the request and app */
        ...
        union
        {
            ...
            struct tasklet_struct task;   /* tasklet to be scheduled when timer goes off */
        } callback_data;
};
 ...
 *
 * @note this must be done before any other APIs are called.
 */
void rtc_sw_task_init(rtc_sw_task_t * ptask, void (*func)(unsigned long data), unsigned long data);

/*!
 * Stop a task that is running on any CPU.  This is only really different from
 * rtc_sw_task_stop on an SMP system
 *
 ...
 * has already fired a value of 0 will be returned.
 * 
 * @note this may be called from interrupt context.
 */
